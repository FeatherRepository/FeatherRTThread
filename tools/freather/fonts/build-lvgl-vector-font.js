...
const sourceFont = path.join(here, 'cache', 'NotoSansSC-wght.ttf');
  ...
  for (let lead = range[0]; lead <= range[1]; lead++) {
    for (let trail = 0xa1; trail <= 0xfe; trail++) {
      ...
      if (chars.length === 1 && chars[0] !== '\ufffd') {
        ...
        if (cp <= 0xffff) codepoints.add(cp);
      }
    }
  }
}
...
` * Source: official Noto Sans SC; GB2312 plus the complete product UI subset. */\n` +
