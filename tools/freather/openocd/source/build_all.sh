...
OPENOCD_BRANCH="${CI_COMMIT_BRANCH:-${CI_MERGE_REQUEST_SOURCE_BRANCH_NAME:-}}"

if [ -z "${OPENOCD_BRANCH}" ]; then
    echo "[ERROR] Unable to determine OPENOCD_BRANCH: CI_COMMIT_BRANCH and CI_MERGE_REQUEST_SOURCE_BRANCH_NAME are both empty." >&2
    exit 1
fi
