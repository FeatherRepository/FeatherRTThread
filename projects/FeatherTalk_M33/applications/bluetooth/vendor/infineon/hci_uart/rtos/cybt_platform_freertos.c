...
static uint32_t platform_irq_lock_depth;
static uint32_t platform_irq_saved_primask;
static volatile uint32_t stack_timer_sequence;
static volatile uint32_t stack_timer_fired_sequence;
static volatile cy_time_t stack_timer_last_delay_ms;
/******************************************************************************
...
static cybt_result_t cybt_enter_autobaud_mode(void);
...
{
    stack_timer_fired_sequence = stack_timer_sequence;
...
void cybt_platform_timer_diagnostics_on_dispatch(void)
{
    cy_time_t delay_ms = stack_timer_last_delay_ms;

    if(delay_ms >= 5000U)
    {
        MAIN_TRACE_ERROR("BT timer dispatch: seq=%lu delay=%lu ms",
                         (unsigned long)stack_timer_fired_sequence,
...
void cybt_platform_disable_irq(void)
{
    ...
    __disable_irq();
    if(platform_irq_lock_depth == 0U)
    {
        platform_irq_saved_primask = primask;
    }
    ++platform_irq_lock_depth;
}

void cybt_platform_enable_irq(void)
{
    uint32_t entry_primask = __get_PRIMASK();

    __disable_irq();
    if(platform_irq_lock_depth == 0U)
    {
        __set_PRIMASK(entry_primask);
        return;
    }
    ...
    if(platform_irq_lock_depth == 0U)
    {
        __set_PRIMASK(platform_irq_saved_primask);
    }
}
...
void cybt_platform_set_next_timeout(uint64_t abs_tick_us_to_expire)
{
...

    {
        ...
        stack_timer_last_delay_ms = next_timeout;
        ++stack_timer_sequence;
        if(next_timeout >= 5000U)
        {
            MAIN_TRACE_ERROR("BT timer arm: seq=%lu delay=%lu ms",
                             (unsigned long)stack_timer_sequence,
...
#ifdef COMPONENT_55500
    result = cybt_enter_autobaud_mode();
    CYBT_RSLT_CHECK(result);
#else
...
static cybt_result_t cybt_enter_autobaud_mode(void)
{
    ...
    result = platform_hal_gpio_init_wrapper(BT_UART_RTS);
    CYBT_RSLT_CHECK(result);

    /* Pull RTS line low */
    result = platform_hal_gpio_write_wrapper(BT_UART_RTS, false);
    CYBT_RSLT_CHECK(result);
    ...
    result = platform_hal_gpio_free_wrapper(BT_UART_RTS);
    CYBT_RSLT_CHECK(result);
...
cybt_result_t cybt_platform_hci_close(void)
{
    ...
    platform_hal_uart_deinit_wrapper();
