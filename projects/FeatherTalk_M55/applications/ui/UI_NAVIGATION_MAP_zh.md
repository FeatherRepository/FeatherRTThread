...
| Wi-Fi | 总开关、被动扫描列表、密码连接、断开、SSID/IP/网关/MAC/RSSI | M55 WHD + SDIO0；后台串行操作；初始化/忙时禁用相应操作；初始化失败显示错误码并保持不可用，不无限等待 | 本阶段不落盘保存密码；退出表单清空密码 |
...
- 使用被动扫描；国家码按 2026-09-04 用户后续明确要求恢复 AU。CN 曾返回初始化错误
  `33556434`，当前 SDK CLM 仅报告 AU/US；此限制仍保留记录。初始化失败时显示真实错误并
  禁用开关/扫描，不能永久显示初始化中，也不自动切换国家码。连接验收见 `../WIFI_BRINGUP_zh.md`。
...
- 连接使用后台扫描描述符中的真实加密类型，不再只传 SSID 导致 UNKNOWN 被误判 WEP；
  没有缓存时后台补扫描，未知安全类型/未发现目标明确失败，不降级为开放网络。
- AU 下双频两轮连接/地址/网关/启停通过 38 项检查，UI 回归 55 项通过；实际标签、IP 和
  按钮状态在线回读通过。短时 TCP + UI + 空闲蓝牙通过不等于全业务并发或吞吐性能达标。
