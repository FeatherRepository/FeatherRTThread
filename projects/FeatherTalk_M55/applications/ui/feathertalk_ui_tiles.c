...
#define FT_TILE_EDIT_SCALE        264
#define FT_TILE_HANDLE_SIZE        29
#define FT_TILE_HANDLE_HIT_PAD      8
#define FT_TILE_HANDLE_EXT_DRAW    32
...
static void tile_scale_anim_cb(void *object, int32_t value)
{
    if (object_valid((lv_obj_t *)object))
        lv_obj_set_style_transform_scale((lv_obj_t *)object, value, LV_PART_MAIN);
}
                             ...
                             uint8_t columns, uint8_t rows)
{
    ...
    if (occupied == RT_NULL || columns == 0U || rows == 0U ||
        ...
        row + rows > FT_TILE_GRID_ROWS) return false;
                                   ...
                                   uint8_t *result_column, uint8_t *result_row)
{
    ...
    for (row = 0U; row + rows <= FT_TILE_GRID_ROWS; row++)
    {
...
{
    if (tile == RT_NULL || !object_valid(tile->object)) return;
    lv_anim_delete(tile->object, tile_scale_anim_cb);
    lv_obj_set_style_transform_scale(tile->object, 256, LV_PART_MAIN);
}
...
static void tile_animation_start(ft_start_tile_runtime_t *tile)
{
    ...
    lv_obj_set_style_transform_pivot_x(tile->object,
                                       lv_obj_get_width(tile->object) / 2,
                                       LV_PART_MAIN);
    lv_obj_set_style_transform_pivot_y(tile->object,
                                       lv_obj_get_height(tile->object) / 2,
                                       LV_PART_MAIN);
    lv_anim_init(&animation);
    lv_anim_set_var(&animation, tile->object);
    lv_anim_set_exec_cb(&animation, tile_scale_anim_cb);
    lv_anim_set_values(&animation, 256, FT_TILE_EDIT_SCALE);
...
static void update_pattern(ft_start_tile_runtime_t *tile)
{
    ...
    {
        tile->pattern = ft_icon_create(tile->object, tile->pattern_icon,
...
void ft_tiles_apply_opacity(uint8_t global_opacity)
{
    ...
    for (i = 0U; i < s_tile_count; i++)
        if (object_valid(s_tiles[i].object))
            lv_obj_set_style_bg_opa(s_tiles[i].object,
...
static void tile_edit_leave(void)
{
    if (s_selected != RT_NULL)
    {
        ...
        if (object_valid(s_selected->object))
            lv_obj_set_style_border_width(s_selected->object, 0, LV_PART_MAIN);
...
static void tile_edit_enter(ft_start_tile_runtime_t *tile)
{
    ...
    if (object_valid(s_container)) lv_obj_remove_flag(s_container, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_obj_set_style_border_width(tile->object, ft_layout_px(FT_TILE_EDIT_BORDER),
                                  LV_PART_MAIN);
    lv_obj_set_style_border_color(tile->object, lv_color_white(), LV_PART_MAIN);
    lv_obj_set_style_border_opa(tile->object, LV_OPA_80, LV_PART_MAIN);
...
static size_t placeholder_snap_nearest(const lv_point_t *desired_center, bool force)
{
    ...
    visible_rows = span_that_fits(lv_area_get_height(&content_area),
                                  ...
                                  FT_TILE_GRID_ROWS);
    maximum_row = visible_rows > s_selected->row_span ?
                  visible_rows - s_selected->row_span : 0U;
    for (i = 0U; i < s_tile_count; i++)
    {
        uint8_t bottom = s_tiles[i].grid_row + s_tiles[i].row_span;
        if (bottom > maximum_row + s_selected->row_span)
            maximum_row = bottom - s_selected->row_span;
    }
    if (maximum_row + s_selected->row_span > FT_TILE_GRID_ROWS)
        maximum_row = FT_TILE_GRID_ROWS - s_selected->row_span;
...
static bool tile_settle_layout_for_edit(ft_start_tile_runtime_t *tile)
{
    ...
    lv_obj_set_pos(s_placeholder, grid_x(tile->grid_column), grid_y(tile->grid_row));
    lv_obj_set_size(tile->object, tile_width(tile->column_span),
                    tile_height(tile->row_span));
    lv_obj_set_pos(tile->object, grid_object_x(tile, tile->grid_column),
                   grid_object_y(tile, tile->grid_row));
    lv_obj_add_flag(tile->object, LV_OBJ_FLAG_FLOATING);
                         ...
                         ft_tile_handle_id_t handle, const lv_point_t *point)
{
    ...
    lv_obj_get_coords(tile->object, &tile_area);
    /* Each handle owns exactly two edges.  Compute its growth room from the
     * opposite fixed edges to the visible desktop bounds. */
    available_width = handle == FT_TILE_HANDLE_RESIZE_BL ?
                      tile_area.x2 - container_area.x1 + 1 :
                      container_area.x2 - tile_area.x1 + 1;
    available_height = handle == FT_TILE_HANDLE_RESIZE_TR ?
                       tile_area.y2 - container_area.y1 + 1 :
                       container_area.y2 - tile_area.y1 + 1;
    if (available_width < lv_obj_get_width(tile->object))
        available_width = lv_obj_get_width(tile->object);
    if (available_height < lv_obj_get_height(tile->object))
        available_height = lv_obj_get_height(tile->object);
    s_resize_max_columns = span_that_fits(available_width,
                                           layout->tile_column_width,
                                           layout->tile_gap,
                                           layout->tile_columns);
    s_resize_max_rows = span_that_fits(available_height,
                                       layout->tile_height,
                                       layout->tile_gap,
...
static void resize_apply_size(int32_t width, int32_t height)
{
    ...
    /* TR fixes left+bottom, BL fixes right+top, BR fixes left+top. */
    if (s_resize_handle == FT_TILE_HANDLE_RESIZE_TR)
        y = s_press_y + s_press_height - height;
    else if (s_resize_handle == FT_TILE_HANDLE_RESIZE_BL)
        x = s_press_x + s_press_width - width;

    lv_obj_set_size(s_selected->object, width, height);
    lv_obj_set_pos(s_selected->object, x, y);
    lv_obj_set_style_transform_pivot_x(s_selected->object, width / 2, LV_PART_MAIN);
    lv_obj_set_style_transform_pivot_y(s_selected->object, height / 2, LV_PART_MAIN);
    if (object_valid(s_selected->live_host))
    {
        int32_t host_height = height - ft_layout_px(46);
        if (host_height < ft_layout_px(20)) host_height = ft_layout_px(20);
        lv_obj_set_size(s_selected->live_host,
                        width - ft_layout_px(24), host_height);
    }
}
...
static void resize_update(const lv_point_t *point)
{
    ...
    reserve_column = s_resize_handle == FT_TILE_HANDLE_RESIZE_BL ?
                     fixed_right - cover_columns : s_resize_start_column;
    reserve_row = s_resize_handle == FT_TILE_HANDLE_RESIZE_TR ?
                  fixed_bottom - cover_rows : s_resize_start_row;
    final_column = s_resize_handle == FT_TILE_HANDLE_RESIZE_BL ?
                   fixed_right - columns : s_resize_start_column;
    final_row = s_resize_handle == FT_TILE_HANDLE_RESIZE_TR ?
...
static void resize_finish(void)
{
    ...
    s_last_resize_displaced =
        resize_resolve_if_changed(s_selected->grid_column,
                                  ...
                                  s_selected->row_span);
    resize_apply_size(tile_width(s_selected->column_span),
                      tile_height(s_selected->row_span));
    update_pattern(s_selected);
    s_interaction = FT_TILE_INTERACTION_NONE;
    (void)tile_settle_layout_for_edit(s_selected);
    tile_animation_start(s_selected);
}
...
static void handle_event_cb(lv_event_t *event)
{
    ...
    if (code == LV_EVENT_PRESSED)
    {
        ...
        if (context->id == FT_TILE_HANDLE_MOVE) move_begin(context->tile, &point);
        else resize_begin(context->tile, context->id, &point);
    }
    else if (code == LV_EVENT_PRESSING)
    {
        if (!event_point(event, &point)) return;
        if (s_interaction == FT_TILE_INTERACTION_MOVE) move_update(&point);
        else if (s_interaction == FT_TILE_INTERACTION_RESIZE) resize_update(&point);
    }
    else if (code == LV_EVENT_RELEASED || code == LV_EVENT_PRESS_LOST)
    {
        if (s_interaction == FT_TILE_INTERACTION_MOVE) move_finish();
        else if (s_interaction == FT_TILE_INTERACTION_RESIZE) resize_finish();
    }
}
...
static void handle_draw_event_cb(lv_event_t *event)
{
    ...
    extent = lv_area_get_width(&area) / 3;
    if (extent < 7) extent = 7;
    head = extent / 2;
    if (head < 3) head = 3;
    ...
    if (context->id == FT_TILE_HANDLE_MOVE)
    {
        /* One centered four-way move symbol: two axes and four arrow heads. */
        draw_handle_segment(layer, &line, center_x, center_y,
                            -extent, 0, extent, 0);
        draw_handle_segment(layer, &line, center_x, center_y,
                            0, -extent, 0, extent);
        draw_handle_segment(layer, &line, center_x, center_y,
                            -extent, 0, -extent + head, -head);
        draw_handle_segment(layer, &line, center_x, center_y,
                            -extent, 0, -extent + head, head);
        draw_handle_segment(layer, &line, center_x, center_y,
                            extent, 0, extent - head, -head);
    ...
    else if (context->id == FT_TILE_HANDLE_RESIZE_BR)
    {
        draw_handle_segment(layer, &line, center_x, center_y,
                            -extent, -extent, extent, extent);
        draw_handle_segment(layer, &line, center_x, center_y,
                            -extent, -extent, -extent + head, -extent);
        draw_handle_segment(layer, &line, center_x, center_y,
                            -extent, -extent, -extent, -extent + head);
        draw_handle_segment(layer, &line, center_x, center_y,
                            extent, extent, extent - head, extent);
        draw_handle_segment(layer, &line, center_x, center_y,
                            extent, extent, extent, extent - head);
...
static void tile_ext_draw_event_cb(lv_event_t *event)
{
    if (lv_event_get_code(event) == LV_EVENT_REFR_EXT_DRAW_SIZE)
        lv_event_set_ext_draw_size(event, ft_layout_px(FT_TILE_HANDLE_EXT_DRAW));
}
                               ...
                               ft_tile_handle_id_t id)
{
    ...
    int32_t tile_padding = ft_layout_px(12);
    int32_t edit_border = ft_layout_px(FT_TILE_EDIT_BORDER);
    int32_t corner_offset;
    lv_obj_t *handle;
    if (handle_size < 25) handle_size = 25;
    if (handle_size > 35) handle_size = 35;
    if ((handle_size & 1) == 0) handle_size++;
    corner_offset = tile_padding + edit_border + handle_size / 2;
    handle = lv_obj_create(tile->object);
    lv_obj_set_size(handle, handle_size, handle_size);
    lv_obj_set_style_radius(handle, handle_size / 2, LV_PART_MAIN);
    lv_obj_set_style_bg_color(handle, lv_color_hex(0x181818), LV_PART_MAIN);
    lv_obj_set_style_bg_opa(handle, LV_OPA_90, LV_PART_MAIN);
    lv_obj_set_style_border_color(handle, lv_color_white(), LV_PART_MAIN);
    lv_obj_set_style_border_opa(handle, LV_OPA_80, LV_PART_MAIN);
    lv_obj_set_style_border_width(handle, 1, LV_PART_MAIN);
    ...
    lv_obj_add_event_cb(handle, handle_draw_event_cb, LV_EVENT_DRAW_MAIN_END,
                        &tile->handle_context[id]);
    if (id == FT_TILE_HANDLE_MOVE)
    {
        lv_obj_align(handle, LV_ALIGN_TOP_LEFT, -corner_offset, -corner_offset);
    }
    else
    {
        if (id == FT_TILE_HANDLE_RESIZE_TR)
            lv_obj_align(handle, LV_ALIGN_TOP_RIGHT, corner_offset, -corner_offset);
        else if (id == FT_TILE_HANDLE_RESIZE_BL)
            lv_obj_align(handle, LV_ALIGN_BOTTOM_LEFT, -corner_offset, corner_offset);
        else
            lv_obj_align(handle, LV_ALIGN_BOTTOM_RIGHT, corner_offset, corner_offset);
    }
                                    ...
                                    ft_start_tile_runtime_t *runtime)
{
    ...
    lv_obj_add_event_cb(tile, tile_ext_draw_event_cb,
                        LV_EVENT_REFR_EXT_DRAW_SIZE, RT_NULL);
    lv_obj_refresh_ext_draw_size(tile);
    ...
    icon = ft_icon_create(tile, descriptor->app.app_icon,
                          ft_layout_icon_size(48U), false);
    lv_obj_align(icon, LV_ALIGN_TOP_LEFT, 0, 0);
    runtime->label = lv_label_create(tile);
    ...
    {
        runtime->live_host = lv_obj_create(tile);
...
bool ft_tiles_test_handle_geometry(void)
{
    ...
    int32_t tile_ext_draw = 0;
    int32_t container_ext_draw = 0;
    size_t i;
    ...
    if (!object_valid(s_placeholder) ||
        ...
        return false;
    (void)lv_obj_send_event(s_selected->object, LV_EVENT_REFR_EXT_DRAW_SIZE,
                            &tile_ext_draw);
    (void)lv_obj_send_event(s_container, LV_EVENT_REFR_EXT_DRAW_SIZE,
                            &container_ext_draw);
    if (tile_ext_draw < ft_layout_px(FT_TILE_HANDLE_EXT_DRAW) ||
        container_ext_draw < ft_layout_px(FT_TILE_HANDLE_EXT_DRAW)) return false;
    lv_obj_update_layout(s_container);
    ...
    for (i = 0U; i < FT_TILE_HANDLE_COUNT; i++)
    {
        ...
        expected_x = (i == FT_TILE_HANDLE_MOVE || i == FT_TILE_HANDLE_RESIZE_BL) ?
                     tile_area.x1 : tile_area.x2;
        expected_y = (i == FT_TILE_HANDLE_MOVE || i == FT_TILE_HANDLE_RESIZE_TR) ?
                     tile_area.y1 : tile_area.y2;
        dx = center_x >= expected_x ? center_x - expected_x : expected_x - center_x;
        dy = center_y >= expected_y ? center_y - expected_y : expected_y - center_y;
...
bool ft_tiles_test_move_scrolled(size_t app_index)
{
    ...
    if (app_index >= s_tile_count || !object_valid(s_container) ||
        ...
        test_row + s_tiles[app_index].row_span > FT_TILE_GRID_ROWS) return false;
    ...
    lv_obj_get_coords(s_tiles[app_index].object, &before_raise);
    ...
    if (applied_scroll != 0)
    {
        ...
        valid = before_raise.x1 == after_raise.x1 &&
                before_raise.y1 == after_raise.y1 &&
...
bool ft_tiles_test_layout_settled(void)
{
    ...
    if (!lv_obj_has_flag(s_selected->object, LV_OBJ_FLAG_FLOATING) ||
        ...
        lv_anim_get(s_selected->object, tile_scale_anim_cb) == RT_NULL) return false;
...
bool ft_tiles_test_resize(size_t app_index, uint8_t columns, uint8_t rows)
{
    ...
    moved.x = tile_width(columns) - initial_width;
    moved.y = tile_height(rows) - initial_height;
    resize_begin(&s_tiles[app_index], FT_TILE_HANDLE_RESIZE_BR, &start);
    resize_update(&moved);
    resize_finish();
    return s_tiles[app_index].column_span == columns &&
...
bool ft_tiles_test_resize_anchors(size_t app_index)
{
    ...
    for (i = 0U; i < sizeof(handles) / sizeof(handles[0]); i++)
    {
        ...
        lv_obj_set_size(tile->object, original_width, original_height);
        lv_obj_set_pos(tile->object, original_x, original_y);
        ...
        if (handles[i] == FT_TILE_HANDLE_RESIZE_TR)
            moved.y = -moved.y;
    ...
    lv_obj_set_pos(tile->object, original_x, original_y);
    lv_obj_set_size(s_placeholder, original_width, original_height);
    lv_obj_set_style_transform_pivot_x(tile->object, original_width / 2, LV_PART_MAIN);
    lv_obj_set_style_transform_pivot_y(tile->object, original_height / 2, LV_PART_MAIN);
    if (object_valid(tile->live_host))
    {
        int32_t host_height = original_height - ft_layout_px(46);
        if (host_height < ft_layout_px(20)) host_height = ft_layout_px(20);
        lv_obj_set_size(tile->live_host,
                        original_width - ft_layout_px(24), host_height);
    }
    lv_obj_move_to_index(tile->object, -1);
    lv_obj_update_layout(s_container);
    tile_animation_start(tile);
    ...
    for (i = 0U; i < s_tile_count; i++)
    {
        if (!object_valid(s_tiles[i].object) ||
            ...
            s_tiles[i].grid_column + s_tiles[i].column_span >
                ft_layout_get()->tile_columns ||
            s_tiles[i].grid_row + s_tiles[i].row_span > FT_TILE_GRID_ROWS ||
                              ...
                              uint8_t opacity, ft_icon_id_t pattern_icon)
{
    ...
    lv_obj_set_style_bg_opa(tile->object,
