...
更新时间：2026-09-04（下方日期条目为逐次验收记录，早期表格不代替最新硬件状态）
...
## 2026-09-04 SDK Wi-Fi 移植与设置页

- 复用 SDK M55 Wi-Fi demo 的 WHD/SDIO0/lwIP 和仓库内固件资源；无线资源编入 M55 固件，不占用末尾 2 MiB 用户磁盘。M33 继续负责蓝牙，共用模组电源不随单独 Wi-Fi 关闭而切断。
- 设置 → Wi-Fi 已接后台请求队列：开关、被动扫描、真实 AP 列表、密码表单、连接/断开和网络状态。状态栏与通知面板共享同一真实状态；扫描/连接不在 LVGL 线程执行，列表只在扫描完成后更新。
- 修复 DTCM 网络池溢出、512 B SDIO IRQ 栈溢出、RT-Thread 5 消息队列返回值误判、WLAN/netdev 名称不一致等移植问题。保留栈保护，并给资源分区和最小栈加入构建检查。
- 双核构建/烧录通过；三轮开关/扫描的 21 项断言通过，蓝牙 Host 的 IPC enabled 标志未变（此阶段未直接验证 HCI 活性）；Wi-Fi 页两次自动测试共 70 项通过。扫描并行全屏压力测试 34.03 FPS、60 帧/60 GPU 提交、无 scanout timeout。显存检查中文与布局正常，壁纸及偏好校验未变。
- 还未进行认证、DHCP、ping 和吞吐验收，需要先确认实物国家/地区与测试网络；当前没有密码持久化或自动重连。蓝牙 IPC stop 尚未实现，反向共存与全系统并发不算通过。
- 详细架构、复现命令、固件哈希和待办见 `../WIFI_BRINGUP_zh.md`；导航契约已同步 `UI_NAVIGATION_MAP_zh.md`。

## 2026-09-04 Wi-Fi / 蓝牙运行时共享资源与并发启动回归

- 不再用功能宏隔离另一 Host 的启动。新增 Common Radio Manager：M33 单核启动阶段统一配置共享电源和两路复位，再释放 M55；运行时 M55 只拥有 WLAN，M33 只拥有 BT，GPIO 原子位写避免同端口跨核配置覆盖。
- 每个域分别记录占用、状态、复位次数和错误，128 B 共享状态在双核链接器显式预留。普通 Wi-Fi off 执行 WLC_DOWN、保留共享电源；错误核或非法复位请求在接触 GPIO 前拒绝。M55 显示/音频启动不再改无线引脚，M33 延后的板级初始化不再切断公共电源。
- 并发启动实测发现 IPC 额外软件信号量可能释放失败并永久占用，导致界面状态陈旧，但 HCI 本身仍工作。移除该多余全局锁，使用 PDL 目的通道原子锁和短本核临界区；补齐整帧 cache 维护。`bt_status` 明确区分 Host enabled 与实时控制器探测。
- 最终双核构建、烧录和校验通过。BT HCD 下载过程中关闭 Wi-Fi，BT 仍完成广播初始化。三轮 Wi-Fi 开关、关闭时拒绝扫描、重新开启/被动扫描通过 136 项主机断言，另有 12 项资源越权断言通过。HCI Command Complete 35→51、广播应答 7→23；两域复位次数各保持 1，BT 硬件错误和 UART timeout 均为 0。
- M33 IPC tx 111→255、rx 32→80，M55 状态持续更新；M33 IPC 错误计数 2→2，即本轮无新增，不宣称启动以来零错误。Wi-Fi 页面 35 项交互断言通过，最终恢复 Home。Flash 壁纸 checksum=0x8f45aa87、偏好 generation=26 均保持不变。
- 仍未验收 Wi-Fi 认证/DHCP/吞吐、连接态蓝牙音频并发、BT stop/start 完整回收，以及单核非协调复位恢复；按实际国家地区和授权网络确认、反向启停、并发流量、独立故障恢复顺序推进。
- 完整设计、实板结果、固件 SHA-256 和复现入口见 `../../../FeatherTalk_Common/RADIO_COEXISTENCE_zh.md`；复用脚本为 `tools/freather/test-wifi-bluetooth-isolation.py`，导航图资源所有者表同步更新。

## 2026-09-04 蓝牙/Wi-Fi 双向独立开关

- 接续上节：M33 BTstack 后端已补全异步关闭/重开，不再把 Radio Manager 的 GPIO release
  当成整个蓝牙服务已经停止。单一常驻 owner 执行 HCD、HCI、UART 和 profile 生命周期，协议栈
  只初始化一次；回收定时器、UART 中断及数据源后只释放 BT 域，公共供电和 WLAN 不受影响。
- 设置 → 蓝牙由页面持有 200 ms 状态监视，回读真实 ON/OFF/PENDING/FAILED 与 BLE/Classic
  连接状态，pop 时删除监视并清除对象。通知面板同步处理状态；只禁用正在处理的那项开关，
  不阻塞另一无线功能。重复目标不重新下载，HCD 中可取消，快速交替请求不会被旧完成状态覆盖。
- Wi-Fi 重复开关跳过冗余 WLC 命令，关闭后清除陈旧关联与地址；worker 完成后立即更新状态。
  保留原运行 3 Mbps 蓝牙链路和远程版本的 A2DP 修复，关闭阶段拒绝迟到音频帧并通知 M55 关流。
- 增加 `tools/freather/test-radio-lifecycle.py`；首版三循环、下载取消/恢复通过 142 项断言。
  最终扩展版另检验一次协议栈初始化、下载期间 Wi-Fi 扫描、快速请求，以及从匹配 ELF 动态解析
  实际 LVGL 标签和按钮状态，验证蓝牙页面不重进即可刷新。最终结果与双核哈希统一记录在
  `../../../FeatherTalk_Common/RADIO_COEXISTENCE_zh.md`，导航图同步本轮异步状态和释放契约。
- 仍需真实 AP 的地区/认证/DHCP/吞吐验证及已连接 BLE/GATT/A2DP 的交叉启停和并发压力测试。
  本次没有建立任何新配对或 AP 连接，不宣称射频/音频并发已经验收；备用 AIROC 关闭仍不支持。

## 2026-09-04 全局键盘比例与全部输入入口

- 根因：Wi-Fi 密码页与文件名弹窗直接给键盘 `flex_grow=1`，高度由容器剩余空间决定；文件名
  弹窗自身还固定为 88% 屏高。它们没有遵循搜索页的受限键盘区域，出现键盘被撑大且各页不一致。
- 新增公共 `feathertalk_ui_keyboard.c/.h`，所有活动产品键盘只通过 `ft_ui_keyboard_create()`
  创建；按键区默认占屏高 30%，不超过去掉系统栏后的可用高度一半，横竖屏布局使用同一计算。
  全局比例只在 `feathertalk_ui_layout.c` 的 `FT_KEYBOARD_SCREEN_PERCENT` 定义一次。
- 全部入口：全局应用搜索、设置搜索、Wi-Fi 密码、新建文件夹、重命名。480×800 屏幕各按键区
  均为 240 px；搜索/设置额外显示收起栏，不挤小键区。公共组件默认禁止 flex-grow，页面只管理
  定位、显示/隐藏和确认/取消语义。不存在某页单独修改百分比的分支。
- Wi-Fi 表单改成上方可滚动表单 + 固定底部键盘，表单滚动不移动键盘，收起后归还表单可用空间。
  文件名弹窗按内容收紧并停靠导航栏上方，错误信息变长也不拉伸键盘。
- `check-ui-navigation-map.py` 在构建前检查产品 `.c/.h`，只允许公共组件存在一个底层 LVGL
  键盘构造器调用；后续新增页面绕过默认设置会使构建失败。导航图同步所有入口及此约束。
- 新增不写文件的 `feather_keyboard_test`，五轮验证搜索/设置、新建/重命名、反复展开、符号/
  数字模式、错误文本与弹窗释放；Wi-Fi 回归新增固定高度、收起空间恢复和滚动/模式切换断言。
  布局模型覆盖 200×240、240×320、320×480、480×800、720×1280、800×480。
- 实板验证：`feather_keyboard_test` 81 项、`feather_wifi_ui_test` 55 项，合计 136 项检查全部
  通过；当前 480×800 屏幕各入口的实际 LVGL 键盘高度均为 240 px。其他分辨率仅通过布局
  模型检查，不宣称已进行对应屏幕的触摸验收。构建导航检查通过，并验证绕过公共工厂会报错。
- 运行中 SWD 回读曾出现横线；首次所谓稳定回读并未成功停核，不作为稳定截图证据。改用
  `targets <core>; halt; wait_halt 2000`，确认两个核心 `curstate=halted` 后重新回读，
  搜索键盘快照无横线且比例正确，随后确认两核恢复 `running`。这只验证本场景，不代表
  已修复所有历史绘制问题。截图：`tmp/keyboard-proportion-search-halted.png`。
- M55 构建、官方 OpenOCD 烧录和校验完成；本轮仅变更 M55，M33 保持上一轮镜像。最终回到
  Home，两个无线资源均 READY、各复位 1 次、资源错误为 0；偏好 generation=26、writes=0/0、
  壁纸 checksum=0x8f45aa87 不变。测试不创建/重命名文件、不连接 AP、不配对；Wi-Fi 日志曾有
  `rt_wlan_get_mac` 获取失败，未把本轮键盘检查当作完整网络验收。
