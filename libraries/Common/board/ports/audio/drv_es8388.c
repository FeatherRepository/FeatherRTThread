...
rt_err_t es8388_init(const char *i2c_name, rt_uint16_t pin)
{
    ...
    reg_write(ES8388_DACCONTROL20, 0x9C); // 右混音器 0 dB.
    ...
    reg_write(ES8388_ADCCONTROL1, 0x00); // MIC Left and Right channel PGA gain
    ...
    //Volume
    reg_write(ES8388_DACCONTROL26, 0x10);
    reg_write(ES8388_DACCONTROL27, 0x10);
...
rt_err_t es8388_start(enum es8388_mode mode)
{
    ...
    if (mode == ES_MODE_LINE)
    {
        ...
        reg_write(ES8388_DACCONTROL17, 0xB8); // left DAC to left mixer enable  and  LIN signal to left mixer enable 0db  : bupass enable
...
void es8388_volume_set(rt_uint8_t volume)
{
    ...
    reg_write(ES8388_DACCONTROL26, volume);//Left output volume
    reg_write(ES8388_DACCONTROL27, volume);//Right output volume
}
...
rt_uint8_t es8388_volume_get(void)
{
    ...
    volume = reg_read(ES8388_DACCONTROL26);
    if (volume == 0xff)
    {
        volume = 0;
    }
